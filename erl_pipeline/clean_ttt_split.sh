#!/bin/bash
# Clean up artifacts from a previous ERL TTT+split pipeline run.
#
# Usage:
#   bash erl_pipeline/clean_ttt_split.sh          # delete artifacts
#   bash erl_pipeline/clean_ttt_split.sh --dry-run # show what would be deleted

set -euo pipefail

# ── Paths (match run_erl_4gpu_ttt_split.sh) ──────────────────
...
ERL_LOG="${SCAFFOLD_DIR}/erl_pipeline/erl_log_ttt_split"
...
for d in "${SCAFFOLD_DIR}"/autoresearch_erl_ttt_split_worker_*; do
    delete "$d"
done
...
if ! $DRY_RUN; then
    echo "Done. Ready for a fresh ERL TTT+split run."
fi
