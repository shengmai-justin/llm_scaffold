#!/bin/bash
# Run the ERL pipeline on 8× RTX Pro 6000 Blackwell with:
#   - TTT-Discover entropic LOO advantages (--adv-type ttt)
#   - Split pipeline (Ideator RL-trained + Implementer frozen)
# See docs/SPLIT_PIPELINE.md and results/2026-04-20_erl_ttt_split.md.
#
# This is the Pro 6000 counterpart of run_erl_4gpu_ttt_split.sh (B200).
# Paths namespaced by `_ttt_split` so it doesn't collide with plain GRPO-split
# or the monolithic Pro 6000 runs.

#SBATCH --job-name=erl-pro6000-ttt-split
#SBATCH --output=autoresearch_erl_pro6000_ttt_split_%j.log
#SBATCH --ntasks=1
...
ERL_REPO="${ERL_REPO:-$SCAFFOLD_DIR/autoresearch_erl_pro6000_ttt_split}"
...
# ── Clone the ERL working repo if missing ────────────────────
if [ ! -d "$ERL_REPO" ]; then
...
export RAY_TMPDIR="/tmp/raye_ttt_split_${USER}"
mkdir -p "$RAY_TMPDIR"
# NOTE: do NOT wipe RAY_TMPDIR here — that would break --resume-step flows.
# Use clean_pro6000_ttt_split.sh for a fresh start.
...
echo "Mode:      ERL Pro 6000 8-GPU TTT SPLIT (model=GPU${MODEL_GPUS}, eval=GPU${EVAL_GPUS})"
...
python erl_main.py \
    ...
    --split-pipeline \
    --attn-impl sdpa \
    --log-dir ./erl_log_pro6000_ttt_split
